...
test('地形(连续场): 锥8角输入→整条带 / 安全区输入→更小 (CORE 两种范围能力都保留; 生产采用安全区)', () => {
  const { mc, regions, frame } = setup();
  const f1 = frame(1);
  const basePts = [mc[1][2], mc[1][3], mc[2][3]];                // 第二层(绿) 底面
  const strip = terrainOnPlane([...mc[1], ...mc[2]], basePts, f1); // 整段锥 8 角
  const safe = terrainOnPlane(regions[1].points, basePts, f1);     // 旧: 安全区点集
