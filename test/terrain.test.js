...
test('行走环线 loopPoint: 全程在安全区 footprint 内, 闭合, 脚贴地形(与 liftAt 同源)', () => {
  const { regs, basePtsOf, fpOf } = setup();
  ...
  for (let t = 0; t < 1; t += 0.05) {
    const { point, perimeter } = loopPoint(m, t);
    assert(perimeter > 0, '周长应>0');
    const lift = liftAt(m, { x: point.x, y: 0, z: point.z }); // 仅取 (x,z) 验证贴地 — 重算基面 y
    // 基面投影在 footprint 内(竖直抬升不会横向出界, 投影基面点即可)
    const yBase = point.y - lift.lift;                        // 由"脚贴地形"反推基面 y
    const bp = { x: point.x, y: yBase, z: point.z };
    const a = dot(sub(bp, m.pa), m.e1), b = dot(sub(bp, m.pa), m.e2);
    assert(inHull2(a, b), `t=${t.toFixed(2)} 环线点应在安全区内`);
  }
