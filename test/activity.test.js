...
test('层地面切分: 地面0 被下一段锥切出 安全碎片+穿帮块, 面积守恒', () => {
  // 实测穿帮: 光线 Σ→M1→M2 后撞上地面0 伸进 M2→M3 锥内的部分 (2026-06 逐像素追迹确认)
  const mc = mcOf();
  const quad = [mc[0][2], mc[0][3], mc[1][3], mc[1][2]];          // 地面0 (M1→M2 底面)
  const s = splitFloorByNextCone(quad, mc[1], mc[2]);
  assert(s.bug && s.bug.length >= 3, '应切出穿帮块(实测穿帮存在, 不该为空)');
  assert(s.safe.length >= 1, '应切出至少一块安全碎片');
  const total = s.safe.reduce((t, f) => t + polyArea(f), 0) + (s.bug ? polyArea(s.bug) : 0);
