...
export const PARAM_SCHEMA = [
  { folder: '镜面 (每面独立)', items: [
    ...
    { key: 'mDist', type: 'vec3', sub: ['Σ→M₁ 距离', 'M₁→M₂ 距离', 'M₂→M₃ 距离'], min: 20, max: 1200, step: 1, def: [254, 359, 634], gh: '14/6', note: '★M₂/M₃为.gh实测; Σ→M₁默认按作者构图调为254(原实测550)' },
  ...
  { folder: '地形', collapsed: true, items: [
    ...
    { key: 'terrainEnd1', label: '层1 结尾抬升 (×层高)', type: 'num', min: 0, max: 0.5, step: 0.01, def: 0.04, note: '坡度基线(链式): 首层起点恒贴地(一条线)→爬到此值; 层2起点自动=层1结尾(σ域冻结带保证严格相等)' },
    { key: 'terrainEnd2', label: '层2 结尾抬升 (×层高)', type: 'num', min: 0, max: 0.5, step: 0.01, def: 0.14, note: '层2从层1结尾起步→爬到此值; 层3起点自动对齐' },
    { key: 'terrainEnd3', label: '层3 结尾抬升 (×层高)', type: 'num', min: 0, max: 0.5, step: 0.01, def: 0.3, note: '末层结尾; 调三个结尾=控制整体坡度走势(噪声起伏叠加在基线之上); 太高会顶穿锥顶' },
  ...
  { folder: '人物', items: [
    { key: 'showFigures', label: '人物 (每层一人)', type: 'bool', def: true, note: '每个有地形的安全区放一人, 同身高(解读A: 人同大·空间逐层放大·受光面各异); 行走时三人各走各层环线' },
    { key: 'figureH', label: '人物身高', type: 'num', min: 10, max: 300, step: 1, def: 80, note: '原始单位 (取景框高默认40作参照); 脚自动贴地形' },
    { key: 'figureWalk', label: '人物行走 (环线)', type: 'bool', def: false, ref: 'figureWalk', note: '沿各层安全区收缩环线匀速行走, 脚贴地形, 关节摆动。开"动态PT"可走动实时看、停走凝固; 关则与PT互斥(走动退PT)' },
