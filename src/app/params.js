...
export const PARAM_SCHEMA = [
  ...
  { folder: '地形', collapsed: true, items: [
    ...
    { key: 'terrainAmp', label: '起伏幅度 (×层高)', type: 'num', min: 0, max: 0.5, step: 0.01, def: 0.2, note: '幅度=层高×系数, 深层自动更大(W₁<W₂<W₃); 太高会挡住后层(作者预言过的遮挡)' },
    ...
    { key: 'terrainEnd1', label: '层1 结尾抬升 (×层高)', type: 'num', min: 0, max: 0.5, step: 0.01, def: 0.07, note: '坡度基线(链式): 首层起点恒贴地(一条线)→爬到此值; 层2起点自动=层1结尾(σ域冻结带保证严格相等)' },
    { key: 'terrainEnd2', label: '层2 结尾抬升 (×层高)', type: 'num', min: 0, max: 0.5, step: 0.01, def: 0.26, note: '层2从层1结尾起步→爬到此值; 层3起点自动对齐' },
    { key: 'terrainEnd3', label: '层3 结尾抬升 (×层高)', type: 'num', min: 0, max: 0.5, step: 0.01, def: 0.5, note: '末层结尾; 调三个结尾=控制整体坡度走势(噪声起伏叠加在基线之上); 太高会顶穿锥顶' },
