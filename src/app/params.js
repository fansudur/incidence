...
export const PARAM_SCHEMA = [
  ...
  { folder: '天空与太阳 (全局)', items: [
    ...
    { key: 'skyEnv', label: '环境天空 (背景+全局补光)', type: 'bool', def: true, ref: 'skyEnv', note: '一键开/关天空: 关=去掉天空背景与全场补光→纯黑宇宙, 只剩太阳/分区灯。PT 里实时生效(比调黑颜色更彻底, 改色只改背景不改采样补光)' },
  ...
  { folder: '分区太阳 (每黄区独立)', collapsed: true, items: [
    { key: 'zoneSun', label: '◉ 开启分区太阳 (自动关全局太阳)', type: 'bool', def: false, ref: 'zoneSun', note: '开=关掉全局太阳, 每个有地形的黄区各一盏远距窄锥聚光灯, 光只落自己那块(锥外为零→不打别区/镜面)。三层的方位/高度/强度各自独立可调。灯本身不可见、不遮挡; 效果在 PT 里看' },
    { key: 'zoneSunAz', type: 'vec3', sub: ['层1 方位°', '层2 方位°', '层3 方位°'], min: 0, max: 360, step: 1, def: [135, 135, 135], note: '每层各自的水平来向 — 想让某层避开背光就单独转它' },
